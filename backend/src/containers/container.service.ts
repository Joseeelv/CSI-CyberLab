...
export class ContainerService {
  ...
  async createContainer(containerData: ContainerDto): Promise<Container> {
    ...
    const image = await this.imageRepository.findOne({
      where: { uuid: containerData.imageId },
      relations: ["operatingSystem"],
    });
    if (!image) {
    ...
    if (containerData.labId) {
      lab = await this.labRepository.findOne({
        where: { uuid: containerData.labId },
      });
      if (!lab) {
    ...
    try {
      ...
      const createdEntity = this.containerRepository.create(
        newContainer as Partial<Container>,
      ) as Container;
